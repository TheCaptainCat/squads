...
async def test_inbox_finds_open_mentions_only(svc):
    t1 = (await svc.create(ItemType.TASK, "open one")).item
    t2 = (await svc.create(ItemType.TASK, "done one")).item
...
async def test_inbox_accepts_at_prefix(svc):
    t = (await svc.create(ItemType.TASK, "t")).item
...
async def test_inbox_surfaces_mention_in_subentity_discussion(svc):
    ...
    feat = (await svc.create(ItemType.FEATURE, "Login feature")).item
    ...
    task = (await svc.create(ItemType.TASK, "Implement reset")).item
    ...
    rev = (await svc.create(ItemType.REVIEW, "Code review")).item
    await svc.add_finding(rev.id, "Null deref")  # F1
