...
async def test_add_operator_rejects_duplicate_slug(svc):
...
async def test_operator_cli_add_list_and_assign(project, invoke):
    add = await invoke(["operator", "add", "Pierre Chat"])
    ...
    upd = await invoke(["task", "3", "update", "--assignee", "op-pierre"])
    assert upd.exit_code == 0
    j = json.loads((await invoke(["task", "3", "show", "--json"])).output)
