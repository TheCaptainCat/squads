...
def test_append_line_writes_valid_jsonl(tmp_path):
...
def test_append_line_fails_gracefully_on_unwritable_path(tmp_path, capsys):
    ...
    append_line(path, ts="2026-06-15T10:00:00Z", actor="system", op="test", target="", delta={})
...
def test_read_lines_multiple_entries(tmp_path):
    path = tmp_path / ".reflog.jsonl"
    for i in range(3):
        append_line(
            path,
...
def test_read_lines_skips_trailing_partial_line_silently(tmp_path, capsys):
...
def test_read_lines_warns_on_interior_malformed_line(tmp_path, capsys):
...
def test_create_emits_reflog_line(svc, frozen_time):
    """Service.create() appends one reflog line with op=create."""
    item = svc.create(ItemType.TASK, "Write tests").item
    lines = read_lines(reflog_path(svc.paths.squad_dir))
...
def test_set_status_emits_reflog_line(svc, frozen_time):
...
def test_update_emits_reflog_line(svc, frozen_time):
    """Service.update() appends one reflog line with op=update."""
    item = svc.create(ItemType.TASK, "T").item
    svc.update(item.id, title="Updated T")
    lines = read_lines(reflog_path(svc.paths.squad_dir))
...
def test_ref_add_emits_reflog_line(svc, frozen_time):
...
def test_remove_emits_reflog_line(svc, frozen_time):
    """Service.remove_work_item() appends one reflog line with op=remove + gone-item snapshot."""
    item = svc.create(ItemType.TASK, "Gone").item
    item_id = item.id
    item_type = item.type.value
    item_title = item.title
    svc.remove_work_item(item_id)
    lines = read_lines(reflog_path(svc.paths.squad_dir))
...
def test_retype_emits_reflog_line(svc, frozen_time):
    ...
    """
    item = svc.create(ItemType.TASK, "Needs retype").item
    old_id = item.id
    result = svc.retype(old_id, ItemType.BUG)
...
def test_repair_emits_reflog_line(svc, frozen_time):
...
def test_actor_flows_into_reflog_line(svc, frozen_time):
    """The ambient actor set via set_actor() appears in the reflog line."""
    actor.set_actor("python-dev")
    item = svc.create(ItemType.TASK, "Authored by python-dev").item
    lines = read_lines(reflog_path(svc.paths.squad_dir))
    create_lines = [ln for ln in lines if ln.op == "create" and ln.target == item.id]
    assert create_lines
...
def test_reflog_not_consulted_by_repair(svc, frozen_time, tmp_path):
...
def test_no_reflog_squad_is_backward_compatible(svc, frozen_time):
    """A squad directory with no reflog file behaves identically (no error)."""
    rpath = reflog_path(svc.paths.squad_dir)
    if rpath.exists():
        rpath.unlink()
    # All read/write ops must still work.
    item = svc.create(ItemType.TASK, "No reflog").item
    assert svc.get(item.id).title == "No reflog"
    # The file was (re-)created by the create op.
    assert rpath.exists()


def test_append_line_swallows_oserror(tmp_path, capsys):
...
def test_append_line_swallows_serialization_error(tmp_path, capsys):
...
def test_failed_reflog_append_does_not_rollback_mutation(svc, frozen_time, monkeypatch, capsys):
    ...
    item = svc.create(ItemType.TASK, "Must exist").item
    # The index committed and the item is readable despite the reflog blowing up.
    assert svc.get(item.id).title == "Must exist"
