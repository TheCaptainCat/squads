...
def _get_role_item_sync(svc: service.Service, slug: str):
    """Sync wrapper for sync test methods (e.g. CLI tests that use CliRunner)."""
    import anyio

    return anyio.run(_get_role_item, svc, slug)
...
class TestActivateRoleWithName:
    async def test_explicit_name_overrides_bundled(self, project, svc):
    ...
    async def test_explicit_name_overrides_resolver_toml_name(self, project, svc):
    ...
    async def test_no_name_uses_bundled_default(self, project, svc):
    ...
    async def test_no_name_uses_toml_full_name(self, project):
        ...
        item = await svc2.activate_role("devops")
        assert item.extra.get(X.FULL_NAME) == "Ops Override"

    async def test_activate_with_name_flag_service(self, project, svc):
        """Service layer: activate_role with name stores it in item."""
        await svc.activate_role("devops", name="Ops Dev")
        found = await _get_role_item(svc, "devops")
        assert found.extra.get(X.FULL_NAME) == "Ops Dev"


class TestInitWithNames:
    async def test_names_kwarg_flows_to_role_item(self, tmp_path, monkeypatch, frozen_time):
        ...
        arch = await _get_role_item(svc, "architect")
        mgr = await _get_role_item(svc, "manager")
        assert arch.extra.get(X.FULL_NAME) == "Ada Lovelace"
        assert mgr.extra.get(X.FULL_NAME) == "Grace Hopper"

    async def test_unnamed_roles_use_bundled_name(self, tmp_path, monkeypatch, frozen_time):
    ...
    async def test_names_written_to_config_toml(self, tmp_path, monkeypatch, frozen_time):
    ...
    async def test_config_toml_round_trips_init_names(self, tmp_path, monkeypatch, frozen_time):
    ...
    async def test_names_none_produces_empty_init_names(self, tmp_path, monkeypatch, frozen_time):
    ...
    async def test_names_flow_to_claude_md_roster(self, tmp_path, monkeypatch, frozen_time):
...
class TestCliInitName:
    def test_name_flag_single(self, runner, tmp_path, monkeypatch, frozen_time):
        ...
        mgr = _get_role_item_sync(svc, "manager")
        assert mgr.extra.get(X.FULL_NAME) == "Grace Hopper"
    ...
    def test_name_flag_multiple(self, runner, tmp_path, monkeypatch, frozen_time):
        ...
        svc = service.Service(resolve())
        mgr = _get_role_item_sync(svc, "manager")
        arch = _get_role_item_sync(svc, "architect")
        assert mgr.extra.get(X.FULL_NAME) == "Grace Hopper"
        assert arch.extra.get(X.FULL_NAME) == "Ada Lovelace"
...
class TestCliRoleActivateName:
    async def test_activate_with_name_flag(self, project, invoke):
        """sq role activate <slug> --name 'Full Name' stores the name in the ROLE item."""
        result = await invoke(["role", "activate", "architect", "--name", "Ada Lovelace"])
        assert result.exit_code == 0, result.output
    ...
    async def test_activate_name_flows_to_claude_md(self, project, invoke):
    ...
    async def test_activate_no_name_uses_bundled(self, project, invoke):
        """sq role activate without --name uses the bundled PREDEFINED full_name."""
        result = await invoke(["role", "activate", "qa"])
        assert result.exit_code == 0, result.output
        svc = service.Service(project)
        qa = await _get_role_item(svc, "qa")
        bundled = next(r for r in PREDEFINED if r.slug == "qa")
        assert qa.extra.get(X.FULL_NAME) == bundled.full_name

    async def test_activate_name_flows_to_pointer_file(self, project, invoke):
