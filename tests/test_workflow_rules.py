...
def test_parent_hint_mentions_refs_for_task():
    assert "feature" in workflow.parent_hint("task")
    assert "sq ref add" in workflow.parent_hint("task")
...
async def test_task_parent_must_be_feature(svc):
    ...
    feat = (await svc.create("feature", "f", parent=epic.id)).item  # feature→epic OK
...
async def test_task_links_bug_via_ref(svc):
    ...
    task = (await svc.create("task", "fix it")).item  # no feature parent (technical/bugfix)
