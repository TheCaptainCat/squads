"""Proves the CLI badge surface (--set value / --badge filter / --min-badge threshold /
--sort / show-panel column) is generic over fields_for(), not hand-wired to priority/
severity — using ADR-323's own "two fields off one collection" reuse example (impact/
urgency both bound to a custom 'level' collection) on a brand-new 'incident' type.
"""
...
async def test_set_value_filter_min_sort_and_column_are_generic(project, invoke) -> None:
    """One end-to-end walk over create/--set/--badge/--min-badge/--sort/show — none of it
    hand-written for impact/urgency; all of it derived from the override's declared fields."""
    _write_override(project.squad_dir)

    # --- create two incidents, no dedicated --impact/--urgency create flag exists (create
    # stays untouched by this task) — set the custom fields generically via --set. ---
    ...
    # --- generic value setting rejects an unknown code for the field's own collection ---
    bad = await invoke(["incident", _num(inc1), "update", "--set", "impact=medium"])
    assert bad.exit_code == 1 and "impact" in bad.output

    # --- show panel: one row per declared field, raw code, generic over fields_for() ---
    ...
    # --- exact filter: --badge CODE=VALUE works for a field the CLI never hard-names ---
    ...
    lo_impact = await invoke(["list", "--type", "incident", "--badge", "impact=low"])
    assert inc2 in lo_impact.output and inc1 not in lo_impact.output

    # --- threshold filter: --min-badge (ordered collection: high=rank0, low=rank1) ---
    # min urgency=high → only items ranked at least as high as "high" (rank <= 0): inc2 only.
    ...
    # min urgency=low → every ranked item qualifies (rank <= 1): both.
    ...
    # --- tree also derives the same generic filter/sort surface ---
...
async def test_bundled_axis_still_uses_the_dedicated_flags(project, invoke) -> None:
    """--priority/--min-priority sugar keeps working unchanged alongside the generic escape
    hatch — the two paths merge into the same underlying filter, not two divergent ones."""
    _write_override(project.squad_dir)
