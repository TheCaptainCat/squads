...
async def test_init_creates_claude_pointers_and_managed_files(project):
...
async def test_claude_md_has_operators_section_and_session_start(project, svc):
...
async def test_manager_role_describes_the_loop(project):
...
async def test_pointer_frontmatter_is_valid_yaml(project):
...
async def test_settings_merge_does_not_clobber(project, svc):
    ...
    for _be in svc._backends():
        await _be.ensure_scaffold(svc._ctx)
...
async def test_claude_md_injection_idempotent(project, svc):
    before = (project.root / "CLAUDE.md").read_text(encoding="utf-8")
...
async def test_claude_md_impersonation_uses_sq_command_not_path(project, svc):
