...
async def test_retype_moves_the_file_updates_frontmatter_and_index(svc):
    task = (await create_item(svc, "task", "Fix crash")).item
...
async def test_retype_rewrites_incoming_refs_children_and_prose_mentions(svc):
    ...
    child = (await create_item(svc, "bug", "child", parent=task.id)).item
    other = (await create_item(svc, "review", "r")).item
...
async def test_retype_refuses_when_a_child_would_become_invalid(svc):
    feat = (await create_item(svc, "feature", "f")).item
    await create_item(svc, "task", "t", parent=feat.id)  # requires a feature parent
...
async def test_retype_refuses_a_drifted_item_and_leaves_the_file_at_its_original_path(
    ...
    task = (await create_item(svc, "task", "Drifted retype target")).item
