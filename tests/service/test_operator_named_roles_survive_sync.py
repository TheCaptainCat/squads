...
Two input paths set a role's name, and they reach the resolver differently.
``test_partial_dev_role_override_is_honoured_by_sync.py`` covers the override-input path, where
a projection that discards ``--name`` still passes clean. This file covers the other: a name set
through ``sq init --name``/``[init.names]`` or `sq role activate --name`, which never creates an
override file at all.

Two invariants have to hold together for such a name to survive:

1. The caller (`_refresh_catalog_extra`) supplies a merge base for a non-dev role, rather than
   an unconditional ``None``.
2. `resolve_role_with_base` honours the base it is given, including when the slug is in
   ``PREDEFINED`` -- so a correct base from (1) actually reaches the merge.

Every test below exercises both at once; an integration test cannot isolate one from the other,
so a red test here points at either site.
"""
...
# --------------------------------------------------------------------------------------------
# The tightest case: one squad, `architect` named only via `--name`, `qa` named via `--name`
# *plus* an override file for `qa` only. One sync. Both names must survive -- wiring only the
# override path through the item leaves `architect` reverted here.
# --------------------------------------------------------------------------------------------
...
async def test_a_rename_through_this_writer_is_visible_and_the_create_entry_survives(
    ...
    skipped = await svc.sync()
    # The override was placed directly on disk, so the pointer this sync started with still
    # named "Ada Lovelace" — genuinely stale relative to the override's declared name, and
    # this sync is what first regenerates it. A "had drifted" report here is currency detection
    # doing its job: the pointer really was out of date the moment before this call.
    assert skipped == [
