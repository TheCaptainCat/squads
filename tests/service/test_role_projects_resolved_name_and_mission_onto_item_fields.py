...
async def test_a_raised_write_rolls_back_in_memory_and_a_retry_from_a_clean_state_succeeds(
    ...
    # The failed write rolled the item back to what disk still held (the bundled name), so the
    # pointer this pass regenerates from that rolled-back item is unchanged from what activation
    # already wrote — no drift on this pass, only the write failure itself.
    assert first_sync == ["simulated write failure"]

    second_sync = await svc.sync()
    # This retry succeeds, so the pointer moves from the bundled name (still on disk from
    # activation — the first pass never got past its own rollback) to "Ada Lovelace" — a
    # genuine currency fix, reported the same way any other drift is.
    assert second_sync == [
        ".claude/agents/architect.md: had drifted — regenerated by this sync (backend: claude_code)"
    ]
    healed = await svc.get(role.id)
...
async def test_an_interrupted_index_commit_is_healed_by_repair_then_a_further_sync_is_silent(
    ...
    # A separate, pre-existing gap this now surfaces rather than causes: the skip above leaves
    # `item` at the stale (rolled-back) index values rather than the disk-truthful ones, so the
    # SAME pass's own backend projection regresses the pointer to the bundled name for one
    # cycle -- currency detection correctly reports that too, on the same sync. Left as a
    # follow-up (the skip-and-rollback path itself is a different task's fix).
    assert (
        second_sync.count(
    ...
    # Frontmatter and the index are exactly as before this sync -- only the pointer above moved.
    ...
    # The second sync's own regeneration (above) had regressed the pointer to the bundled
    # name; this is the sync that first notices and fixes it against the now-repaired index.
    assert third_sync == [
        ".claude/agents/architect.md: had drifted — regenerated by this sync (backend: claude_code)"
    ]
