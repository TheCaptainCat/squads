...
vs. closed items per assignee, with unassigned items grouped under ``None``; ``list_items``'s
``assignee`` filter (the building block `mine` uses for its item-level match) returns exactly
the items assigned to one slug and excludes items assigned to another. Sub-entity-aware `mine`/
`workload` behaviour has its own dedicated test module.
"""
...
async def test_search_matches_title_and_body_but_not_an_absent_term(svc):
    res = await create_item(svc, "task", "Token validation")
    await svc.set_body(res.item.id, "Validate the JWT expiry and signature.")

    by_title, _skipped = await svc.search("token")
    assert [r.item.id for r in by_title] == [res.item.id]
    by_body, _skipped = await svc.search("expiry")
...
async def test_workload_counts_open_and_closed_per_assignee_and_groups_unassigned(svc):
    await create_item(svc, "task", "t1", assignee="manager")
    done = (await create_item(svc, "task", "t2", assignee="manager")).item
    await svc.set_status(done.id, "InProgress")
    await svc.set_status(done.id, "Done")
    await create_item(svc, "task", "unassigned")
...
async def test_search_status_filter_ands_with_the_query_and_type(svc):
    open_task = await create_item(svc, "task", "Retry the payment webhook")
    done_task = (await create_item(svc, "task", "Retry the payment queue")).item
    await svc.set_status(done_task.id, "InProgress")
    await svc.set_status(done_task.id, "Done")
    await create_item(svc, "bug", "Retry the payment gateway")

    only_open_tasks, _skipped = await svc.search("retry", item_type="task", status="Draft")
    assert [r.item.id for r in only_open_tasks] == [open_task.item.id]

    only_done, _skipped = await svc.search("retry", status="Done")
...
async def test_search_hit_regions_distinguish_body_sub_entity_and_discussion_comment(svc):
    feature = (await create_item(svc, "feature", "Login flow")).item
    ...
    results, _skipped = await svc.search("frostbite")
...
async def test_search_result_carries_item_type_and_status(svc):
    task = (await create_item(svc, "task", "Investigate flaky retry")).item
    await svc.set_status(task.id, "InProgress")

    results, _skipped = await svc.search("flaky")
...
async def test_search_snippet_keeps_a_match_near_the_start_of_a_long_line_unchanged(svc):
    task = (await create_item(svc, "task", "Long line term at the start")).item
    filler = "x" * 200
    await svc.set_body(task.id, f"marigold {filler}")

    results, _skipped = await svc.search("marigold")
...
async def test_search_snippet_windows_around_a_match_past_the_old_cap(svc):
    task = (await create_item(svc, "task", "Long line term past the old cap")).item
    filler = "x" * 180
    await svc.set_body(task.id, f"{filler} unicornberry appears here")

    results, _skipped = await svc.search("unicornberry")
    snippet = results[0].hits[0].snippet
...
async def test_search_snippet_windows_around_a_match_near_the_end_of_a_long_line(svc):
    task = (await create_item(svc, "task", "Long line term near the end")).item
    filler = "x" * 300
    await svc.set_body(task.id, f"{filler} tangerine")

    results, _skipped = await svc.search("tangerine")
    snippet = results[0].hits[0].snippet
...
async def test_search_snippet_windows_on_the_first_of_multiple_matches(svc):
    task = (await create_item(svc, "task", "Repeated term across a long line")).item
    filler = "x" * 180
    await svc.set_body(task.id, f"papaya {filler} papaya")

    results, _skipped = await svc.search("papaya")
    snippet = results[0].hits[0].snippet
    assert snippet.startswith("papaya")


async def test_search_snippet_leaves_a_short_line_unwindowed(svc):
    task = (await create_item(svc, "task", "Short line term")).item
    await svc.set_body(task.id, "a quince falls")

    results, _skipped = await svc.search("quince")
    assert results[0].hits[0].snippet == "a quince falls"


async def test_assignee_filter_returns_exactly_the_items_assigned_to_that_slug(svc):
    """``list_items(assignee=...)`` filters strictly by assignee: an item assigned to slug A
    is returned, an item assigned to a different slug B is not — a positive assertion, not
    just that the slug argument itself validates (that's the separate slug-validation
    cluster)."""
    await svc.add_dev("python")  # registers python-dev
    mine = (await create_item(svc, "task", "Mine", assignee="manager")).item
    await create_item(svc, "task", "Not mine", assignee="python-dev")
