...
async def test_an_index_left_lagging_on_a_catalog_field_still_mutates_without_refusing(svc):
    ...
    # That mutation's own projection ran against the still-lagging index (title missing), so
    # the pointer it wrote is genuinely, if briefly, wrong (a blank role title) — this sync is
    # the one that first notices and fixes it, which currency detection correctly reports.
    assert await svc.sync() == [
        ".claude/agents/reviewer.md: had drifted — regenerated by this sync (backend: claude_code)"
    ]
