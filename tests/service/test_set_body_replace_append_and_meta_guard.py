...
skill is rejected the same way (its definition is template-owned and renders on read, so an
authored body would never be shown); a custom (author-defined) skill is the one roster-type
exception and accepts an authored body.
"""
...
async def test_set_body_on_a_system_skill_is_rejected_as_template_owned(svc):
    seeded = await svc.seed_bundled_skills()
    with pytest.raises(SquadsError, match="template-owned and rendered on read"):
