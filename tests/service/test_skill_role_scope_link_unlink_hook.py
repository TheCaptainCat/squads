...
partial-sync hook, which rewrites only that role's pointer entry. The resolved list itself is
never persisted — not in the pointer's own source of truth, not in the role's definition —
so the hook's observable effect on the *resolved* list is via
``Service.resolved_skills_for_role``; the role's own definition text is resolved fresh on
every ``sq role <slug> show`` and never carries the skills list at all.

Contrast with a raw ``add_ref(..., kind="scopes")``: it writes the identical edge but is not
followed by the hook, so the role's pointer stays stale until the next full ``sync()`` —
...
async def test_link_role_resyncs_the_pointer_without_a_separate_sync_call(svc):
...
async def test_unlink_role_resyncs_the_pointer_immediately(svc):
    role = await svc.activate_role("tech-writer")
    skill = await svc.add_skill("Release Runbook")
    await svc.link_role(skill.id, role.id)
    ...
    assert "release-runbook" not in _pointer_path(svc, "tech-writer").read_text(encoding="utf-8")
    resolved = await svc.resolved_skills_for_role("tech-writer")
    assert "release-runbook" not in resolved


async def test_only_the_linked_roles_pointer_is_touched(svc):
...
async def test_one_skill_scoped_to_several_roles_at_once_preloads_all_and_only_those(svc):
    ...
    tech-writer preloads it for all three (resolver + pointer), while a role never linked
    (python-dev) sees no trace of it in either surface."""
