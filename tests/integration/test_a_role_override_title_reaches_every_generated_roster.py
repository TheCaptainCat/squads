...
The class is closed at the root now: the title is not stored on the role item at all, so there
is no copy for the index to lag on. ``roster()`` resolves each role through the catalog on every
call, which is why the assertions below are about the *compiled* files and about the index
carrying no title to be stale — not about two copies agreeing.
"""
...
async def test_an_overridden_title_reaches_the_index_the_frontmatter_and_both_backends(
    ...
    assert _OVERRIDDEN_TITLE not in (tmp_path / "CLAUDE.md").read_text(encoding="utf-8")

    _write_role_override(paths.squad_dir, "reviewer", f'title = "{_OVERRIDDEN_TITLE}"\n')

    # A service opened after the override exists resolves it; sync is the merge point. The
    # pointer was already written under the bundled title at init, so this sync genuinely
    # regenerates it — a currency fix, reported the same way any other drift is.
    ...
    # No stored copy for the index to lag on -- the half that used to lag no longer exists.
    assert "title" not in _index_role_extra(paths, "reviewer")
...
async def test_a_repair_after_the_sync_changes_nothing_about_the_roster(tmp_path, monkeypatch):
    ...
    assert _index_role_extra(paths, "reviewer") == extra_before
