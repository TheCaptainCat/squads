...
async def test_skill_show_labels_a_bundled_skill_as_system_in_text_and_json(
    seeded_paths, invoke
) -> None:
    svc = service.Service(seeded_paths)
    (sk,) = (await svc.list_items(item_type="skill"))[:1]
    ...
    r = await invoke(["skill", str(sk.sequence_id), "show", "--json"])
    assert json.loads(r.output)["system"] is True


async def test_skill_show_labels_a_custom_skill_as_custom_in_text_and_json(
    seeded_paths, invoke
) -> None:
    svc = service.Service(seeded_paths)
    skill = await svc.add_skill("Release Runbook", description="Ship a release safely.")

    r = await invoke(["skill", str(skill.sequence_id), "show"])
    assert r.exit_code == 0, r.output
    assert "custom (authored)" in r.output

    r = await invoke(["skill", str(skill.sequence_id), "show", "--json"])
    assert json.loads(r.output)["system"] is False


async def test_skill_body_sets_and_re_edits_a_custom_skills_authored_body(
    seeded_paths, invoke
) -> None:
    svc = service.Service(seeded_paths)
    skill = await svc.add_skill("Release Runbook", description="Ship a release safely.")
    ...
    r = await invoke(
        ["skill", str(skill.sequence_id), "body", "-m", "## Instructions\n\nCut the branch."]
    )
    assert r.exit_code == 0, r.output
    assert await svc.read_body(skill.id) == "## Instructions\n\nCut the branch."

    r = await invoke(["skill", str(skill.sequence_id), "body", "-m", "Then publish.", "--append"])
    assert r.exit_code == 0, r.output
    assert (await svc.read_body(skill.id)).endswith("Cut the branch.\n\nThen publish.")


async def test_skill_body_on_a_bundled_skill_is_rejected_with_a_clear_message(
    seeded_paths, invoke
) -> None:
    svc = service.Service(seeded_paths)
    (sk,) = (await svc.list_items(item_type="skill"))[:1]
    ...
    assert "regenerated by `sq sync`" in r.output
