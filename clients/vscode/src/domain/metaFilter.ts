/**
 * Filter state for the meta/roster view (`domain/metaView.ts`, `metaTreeDataProvider.ts`):
 * hide-archived-by-default and a single-status restriction. Deliberately smaller than the work
 * tree's `ListFilter`/`ViewState` (`domain/listView.ts`) — no type filter or grouping, since the
 * roster's 3 fixed buckets already are the type dimension (`domain/reservedTypes.ts`).
 *
 * The two states interact: a status filter always wins over hide-archived, mirroring the `sq`
 * CLI's own rule that `--status` reveals a matching closed/hidden item even without `--all`
 * (`sq list --help`) — so filtering to a hidden-role status (e.g. the bundled `Archived`) shows
 * it, rather than the two settings combining into an unconditionally empty tree.
 */
...
export const DEFAULT_META_VIEW_STATE: MetaViewState = { showArchived: false, statusFilter: null };
