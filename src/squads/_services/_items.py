...
class ItemsMixin(ServiceCore):
    ...
    async def update(  # noqa: PLR0913 — the one metadata entry point
        ...
        *,
        ...
        if self.spec.item_is_roster(item.type) and item.type != META_OPERATOR:
    ...
    async def set_body(self, item_id: str, body: str, *, append: bool = False) -> Item:
        ...
        (author-defined) skill is the one meta-type exception: its body is authored content with
        ...
        def mutate(text: str, item: Item) -> str:
            ...
            elif self.spec.item_is_roster(item.type) and item.type != META_OPERATOR:
                raise SquadsError(
