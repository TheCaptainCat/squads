...
def badge_render(
    ...
    rather than crashing (graceful, mirrors :func:`_status_badge`).
    """
    active_spec = spec if spec is not None else bundled_spec()
...
def set_head(
    ...
    *,
    ...
    inner = render(
        ...
        severity=(
            badge_render(resolve_collection(kind, "severity", spec), severity, spec, as_label=True)
            if severity
            else None
        ),
        story=story,
...
def _summary_cells(kind: str, s: SubEntity, spec: WorkflowSpec | None = None) -> list[str]:
    if kind == "finding":
        sev = (
            badge_render(resolve_collection(kind, "severity", spec), s.severity, spec)
            if s.severity
            else ""
        )
