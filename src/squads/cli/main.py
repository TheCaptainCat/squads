...
from squads.models import Item, ItemType
from squads.paths import number_for_id
from squads.service import init as svc_init
...
def init(
    ...
    sp = result.paths
    roles_line = ", ".join(r.extra.get("slug", r.slug) for r in result.roles) or "—"
    lines = [
...
def list_items(
    ...
    svc = get_service()
    items = svc.list(
        type=parse_type(type) if type else None,
...
def tree(root_id: str | None = typer.Argument(None)):
    ...
    svc = get_service()
    all_items = {i.id: i for i in svc.list()}
...
def guide_list():
    svc = get_service()
    guides = svc.list(type=ItemType.GUIDE)
    if not guides:
    ...
    for col in ("ID", "Title", "Status", "Tech"):
        table.add_column(col)
    for it in guides:
        table.add_row(it.id, e(it.title), it.status.value, it.extra.get("tech", ""))
    console.print(table)
