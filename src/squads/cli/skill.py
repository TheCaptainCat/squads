...
def skill_list():
    svc = get_service()
    skills = svc.list(type=ItemType.SKILL)
    if not skills:
    ...
    for it in skills:
        table.add_row(it.id, it.extra.get("slug", it.slug), e(it.title), it.status.value)
    console.print(table)
...
def skill_show(item_id: str = typer.Argument(...)):
    ...
    rows = [
        ...
        f"[bold]slug:[/bold] {it.extra.get('slug', it.slug)}",
        f"[bold]status:[/bold] {it.status.value}",
        f"[bold]file:[/bold] {it.path}",
    ]
    if it.extra.get("when_to_use"):
        rows.append(f"[bold]when to use:[/bold] {e(it.extra['when_to_use'])}")
