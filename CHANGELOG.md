...
- **Derived views: declared, computed projections over the links an item already has.** A view is a `[views.<name>]` section of the workflow spec with three parts and no fourth: a **source** (every item pointing here with a given ref kind, this item's own sub-entities of a kind, or its descendants of a type), a **projection** (`fields`, plus optional `group_by` and `order_by`), and a **presentation** — a Jinja2 template resolved by the view's own name at `templates/views/<name>.md.j2`. `sq workflow views` lists what your spec declares and `sq workflow view <name> <id>` resolves one against an item. Views compose like every other section: add your own, shadow a bundled one field by field, drop one through `[selected]`, and a view naming a ref kind, sub-entity kind or type the merged spec does not declare is refused at load alongside every other cross-reference.

  **Every view is computed on each request, and none is ever written into an item's file.** There is no sink to declare, nothing to regenerate, nothing to commit, and nothing to reconcile when two branches touch the same item — the frontmatter merges as ordinary data and the next read renders the merged result. `--json` emits the projection and skips presentation entirely: `{fields, group_by, groups}`, one uniform shape across every view and every source, so a client can lay out a view it has never seen. Because presentation is an ordinary bundled template, it is adopter-overridable the day it ships — `sq override scaffold views/<name>.md.j2`, then `diff` and `update`, with no view-specific override machinery.

- **A `milestone` item type (`MILE`), for the release or cycle work is aimed at.** A records-type item with its own lifecycle (`Draft → InProgress → Done`, plus `Cancelled`) that tracks whether the target is still being pursued, never how much of its work is finished, and a `target_date` set with `sq milestone <n> update --set target_date=…`. Previously a project's releases and cycles had nowhere to live in squads at all.

  **Membership lives on the work item, not on the milestone.** A feature, task or bug joins by carrying a forward `targets` ref to it — `sq task <n> ref add MILE-<n> --kind targets` — and the milestone file is never rewritten by that: it holds no list of its members, and there is no verb that adds work from the milestone's side. Re-aiming twenty items at a different release rewrites twenty work items and leaves the milestone byte-for-byte unchanged, and the membership list is recovered by inverting those edges rather than stored anywhere it could disagree with them.

  **`sq milestone <n> show` renders the roll-up: members split three ways, each side counted.** **Delivered** is a member that reached its own kind's happy-path terminal — `Done`, `Verified`, `Accepted`, whatever that lifecycle calls it. **Outstanding** is one still open. **Settled without delivering** is one that stopped some other way, cancelled or superseded: a resting state, but not something the milestone can claim. That third bucket is why the split reads each member's declared `settled`/`delivered` properties rather than a status name — a two-way split treats every settled record as finished and never reports zero outstanding once anything is cancelled — and it is also what lets a milestone hold items of several types on several lifecycles and still group them correctly. It is computed on every request, `--json` included, so there is nothing to refresh. squads has no estimation vocabulary and the roll-up reports item counts accordingly — no burndown, no velocity, no sprint length.

- **A `contract` item type (`PRD`): the living record of what the product does for a user, right now.** The functional twin of the ADR set — a decision record is the technical contract, a contract is the functional contract with the user. Its lifecycle is `Draft → Active → Superseded` (plus `Deprecated`, revivable to `Active`), and it hosts no sub-entities: write one contract per capability or user-facing area, and rewrite it in place as the product changes. Features and epics stay historic — each a point-in-time record of a change and its reasoning — so answering "what does this product do today" no longer means replaying every feature in order and applying each later override by hand. A feature links the contract it shapes with an `implements` ref, and `sq contract <n> refs --in` reads back every feature that has shaped one. Where the boundaries between contracts fall is editorial judgement and nothing enforces it.

  **That `implements` edge is typed but not required, and requiring it is opt-in.** The bundled `feature` type declares the rule — an `implements` ref from a feature points at a contract — which is what makes the two ends read as one relationship; whether a delivered feature *must* name the slice it changed is the maintainer's call rather than a bundled default. Plenty of features are internal refactors, test batteries or tooling that implement no user-facing promise, and a rule with no way to record that produces a warning nobody can clear. A squad that does want the obligation names `validators = ["ref_rule_target_present:contract"]` on `feature` in its own `.overrides/workflow.toml`. `sq check` then warns — never blocks, and still exits `0` — on a feature that reaches a delivered status carrying no such ref, and stays inert until the squad holds at least one contract for that warning to point at.

- **Ref kinds are declared workflow vocabulary.** `[ref_kinds]` in `.overrides/workflow.toml` lets a squad add its own edge label, relabel a bundled one, or drop one it does not use — a three-line table (`label`, an optional one-line `hint`) and the kind is usable immediately in `sq <type> <n> ref add <id> --kind …` and listed by `sq workflow ref-kinds`. Engine behaviour binds to a kind's declared semantic `role` rather than to its name, so a renamed dependency kind keeps driving `sq blocked` and a renamed supersession kind keeps driving `sq check`; a kind that declares no role is navigational, which is what a kind you declare gets unless you say otherwise. A kind the merged spec does not declare is still refused by name, and the refusal lists your own accepted set rather than a fixed one.

- **One override contract across every bundled spec document.** `.overrides/roles.toml` joins the workflow spec, the playbook, the per-role `roles/<slug>.toml` and the template overrides as a first-class override kind, and all five now behave alike: `sq override scaffold roles` writes it stamped and with a worked example, `sq override list` reports its kind and drift state, and `sq override diff roles` / `sq override update roles` name their own object the way every sibling kind does. The role catalog document is the whole-document override — the bundle selection, the developer name pool and model defaults, a bundled role reshaped field by field, or a wholly new role, all in one file — where `--role <slug>` remains the narrower per-role file; the two are not interchangeable and `scaffold` distinguishes them so they cannot be confused. Precedence runs bundled catalog → this document → the per-slug file, which still wins for the same field. A declaration that cannot hold together is refused when the spec loads, naming each problem: a role you declare but leave out of every bundle, a bundle naming a slug nothing declares.

  **An unstamped shadowing override is an error for every kind.** Previously the same missing `squads:override-base` stamp was an error on a workflow override and only a warning on a template or per-role override, so whether a squad's exit code noticed depended on which file the stamp was missing from. It is an error everywhere now, and the message names the kind and the command that re-stamps it. Shadowing is the condition: an override that adds something squads does not ship — a template for a type you declared yourself — has no bundled counterpart it could have branched from and is still asked for nothing.

  **Drift is measured against the document an override actually overrides, and against its content rather than its age.** A per-role override's drift used to be checked against the role *body template*, a different artifact with its own stamp and its own drift, so a change to a bundled role's mission drifted nothing while a change to the body's layout drifted every role override in the squad. It is compared with the role catalog now. And a workflow or playbook override used to count as drifted whenever its stamp named any earlier release, whether or not that release touched the document; drift is content-gated now, so an override stamped two releases back against a document neither release changed reports `current` and stays quiet.

  **`sq override diff` shows what the upgrade changed, not that something changed.** The provenance shipped with squads carries the normalized text of every overridable bundled artifact — every template plus the three spec documents — deduplicated across every release, so the Δ-upgrade pane is a real diff between the bundled document at your stamped version and the bundled document today. Where your stamp predates squads' own coverage of that artifact, the pane says so by name, gives the earliest revision it does carry, and shows the partial diff from there — rather than telling you to read the changelog. An unknown-key refusal on any override document now names the running version alongside the accepted key set, which is the half of the message that tells you whether to upgrade or fix the key.

- **Generated agent pointers name commands, never a local path.** A pointer file — `.claude/agents/<slug>.md`, a skill's `SKILL.md` — used to open with an `@`-reference to the role or skill definition's path inside the squad folder. It now carries the agent's identity, the startup commands bound to that agent's own slug, and one command that fetches the full definition: `sq role <slug> show`, `sq skill <slug> show`. The pointer stops depending on a definition file sitting at a resolvable path on the same filesystem as the session, which is what makes it work when the CLI is a client to a server rather than a reader of a local folder. What a pointer holds is unchanged in principle and smaller in practice: what the host must read before an agent can run, plus the commands for everything else.

  **`sq check` now compares each pointer with what the backend would write for it today, not just whether it exists.** A pointer whose content has fallen behind the roster is reported at **warn** and `sq sync` regenerates it. One specific drift is an **error**: a pointer missing the capability restriction its role currently carries, because a stale pointer that still grants authority the squad revoked cannot be repaired from inside the session it governs. A backend that expresses no such boundary, or writes no per-entry file at all, is never reported for either — the comparison runs only against the paths a backend declares for itself.

- **`sq graph --json` names each edge's declared semantic, and no longer drops an edge it can see.** Every node carries `edge_semantic` alongside `edge_kind`: the *role* the edge's kind declares (`dependency`, `preload`, `supersession`) or `null` for a purely navigational one. Branch on that field rather than on a kind's spelling and a diagram or a client keeps working across a rename. Previously traversal silently skipped any edge whose kind the merged spec did not declare — indistinguishable, at the point where it vanished, from an edge you had filtered out on purpose — so dropping a kind from your vocabulary also erased the existing edges spelled with it. Every edge traverses now, declared or not, carrying its stored spelling and a `null` semantic. An explicit `--kind` filter still gates what is shown, and still refuses a kind your spec does not declare, naming your own accepted set.

- **Agent-facing guidance now says item files are read through `sq`, not opened.** The generated `CLAUDE.md`/`AGENTS.md` region, the `squads` skill, each per-type skill and every role pointer state the read side of the sq-managed rule explicitly: an item is read with `sq <type> <n> show` (`--full --comments` for the whole dossier), never by opening its `.md`, because the command resolves state the file does not carry and a direct read returns strictly less. The write half of that rule was already stated; the read half was assumed, and agents read the files. The same guidance also points at `comment --file` for anything carrying a backtick or a fenced code block.

  **The agent operating manual gains a section on the init-time interview** — the short conversation a coordinating agent can open a fresh squad with, about how this team wants work run, closing by writing the answers into a skill scoped to the roles they apply to. It offers seven areas worth asking about and is explicitly a suggestion: nothing prompts for it, no command runs it, no file is scaffolded from it and nothing checks that it happened.

- **`sq <type> <n> comment` and a sub-entity's `comment` take `--file`.** Read the comment body from a file, or from stdin with `--file -`, instead of assembling it out of repeated `-m` paragraphs. This is the same input shape `body` already had, and it is the reliable way to record a comment containing backticks, quotes or newlines.

### Fixed

- **Dropping a bundled item type through `[selected].items` no longer breaks the squad.** A type that survives the drop can carry declarations naming the one you removed — a ref rule targeting it, a currency validator parameterised on it — and those declarations then referred to vocabulary that no longer existed, so every command in the squad refused to load a spec that `sq workflow lint` had just called clean. The loader now strips a surviving type's declarations that target a dropped type, the same courtesy a dropped type's own attached view already received, so you can remove a type you do not use without hunting down the unrelated-looking type block that mentions it. Only declarations that *target* the dropped type are stripped; parent relationships are untouched.

- **A stale index encoding is no longer reported as if your file had diverged.** When the index held a non-canonical spelling of an item's refs and the file on disk held the identical bytes, `sq check` and the write-seam refusal both said the frontmatter had diverged from the index — a claim you could disprove by opening the file, which is the worst thing a diagnostic can do. Both surfaces now say the index holds a non-canonical encoding, not a divergence, and name the fields. A genuine divergence, and a pre-0.2 `extra.ref_kinds` map the index never folded, keep the wording they had. `sq repair` also rewrites such a file to the canonical encoding rather than only correcting the index, so the correction survives the next write; the reflog entry names each item it rewrote, and a corpus already canonical is not written at all.

### Changed

- **`sq repair` removes stored content that squads now computes on every read.** Two marker-region families go from any item file that carries them, and a role item loses two more things:

  - **the sub-entity roll-up table, and each sub-entity block's badge line** — snapshots of state the item's frontmatter already holds, rewritten on every change and stale the moment two branches touched the same item. `sq <type> <n> show` renders both from the frontmatter on every read.
  - **a role's stored body** — a rendered copy of its definition, emptied. `sq role <slug> show` renders the definition fresh on every call from the role catalog and your own role overrides, so nothing reads the stored copy. Make sure a role you defined yourself still has its `.overrides/roles/<slug>.toml`: a role item that resolves against neither the catalog nor an override has nothing left to render from once its stored copy is gone.
  - **the `extra` keys a role item mirrored from that same definition** — `full_name`, `title`, `mission`, `responsibilities`, `agreements`, `color`, `can_spawn`, `description`, and the `skills` cache. Nothing has written any of them since the definition became a read-time render, and a squad created on this version stores none of them.

  **What a role keeps.** Its `slug`; its default-role designation, if it holds one; and, on a developer role, its `model`, `is_dev` and `tech` — `sq dev add` and `sq role set-default` still write those, and a developer's model is an operator setting the catalog has no answer for. A **non**-developer role's stored `model` goes with the rest, because nothing writes one for that shape. Everything else on a role item is untouched: its own top-level `title`, `status`, `author`, timestamps and refs.

  **A role's body is the only body touched, and it is emptied rather than deleted:** the `sq:body` marker pair stays and only its contents go. **No skill body is touched at all** — not one you authored, and not a template-owned one either. Whether a skill is template-owned is a question about today's vocabulary, and a stored body was written under an earlier one, so the two cannot be told apart on disk; a stored body on a bundled or `sq-<type>` skill is simply left where it is, inert, since `sq skill <slug> show` renders that definition from its template regardless. Every other item's body, every sub-entity's body and discussion, every operator item, and every frontmatter field on anything that is not a role are all untouched too.

  **Every command that rebuilds the index from the files performs the removal**, not just `sq repair`: `sq adopt`, `sq renumber`, and the rebuild `sq migrate up` ends with all reach the same sweep, so on a corpus that carries this content you get a **content diff** alongside the index work you ran the command for. Run any of them on a clean working tree, because none can separate its changes from yours. All four print the same one line counting the item files they rewrote, and name those items in the reflog — `sq repair`, `sq adopt` and `sq migrate up` on their `repair` entry, `sq renumber` on its own, by their post-shift ids. That is a count and a list of ids, not the change itself, so read `git diff` for what actually moved. A squad carrying none of this is untouched, no file is written, and a second run is always a no-op.

  **One behaviour changes with the bytes:** text that lived only in a removed region is no longer matched by `sq search` — a role's mission and responsibilities, and a sub-entity's badge line. `sq role <slug> show` answers for the first, `sq <type> <n> show` and `sq list` for the second.

### Migration

**Schema 0.11 → 0.14 — run `sq migrate up` on a clean working tree.** The two new item types are declared in the bundled spec already, and the runner supplies the two things a declaration alone never puts on disk: each type's own folder, and its generated agent-facing surface — the managed `sq-contract` and `sq-milestone` skills, their pointers, and the compiled `CLAUDE.md`/`AGENTS.md` regions that reference them. Every write the runner itself makes is either a path that did not exist or a region it is the first author of, and running it twice changes nothing further.

**Existing items are rewritten, by the index rebuild the command ends with.** That rebuild is the same sweep described under **Changed** above: the retired sub-entity regions removed from any item file carrying them, and every role body emptied with its mirrored `extra` keys dropped. The command prints how many item files it rewrote and names them in the reflog; review the diff before committing. `sq migrate chlog` describes the same removal in the runbook it prints.

**Run `sq sync` afterwards.** The migration deliberately does not touch any existing role's own pointer, so a role that interacts with either new type — the bundled playbook gives both to the architect, the product owner, the tech lead and every developer — still carries its pre-upgrade preload list until you sync. `sq check` says so in the meantime. On a squad that does not customise role or skill scoping, the sync converges to the same result either way.

## [0.13.1]

### Added

- **`sq workflow lifecycles --json` publishes every declared state machine, and the `lifecycle` reference on the type and sub-entity-kind catalogs now has somewhere to resolve.** One row per lifecycle: `initial` (the starting status), `states` (every status the machine declares, in breadth-first order from `initial` — documented and deterministic), and `transitions` (every allowed move, as `{from, to}` pairs, in that same order). Join a type's or a sub-entity kind's own `lifecycle` field to this catalog's identity key to answer "what statuses can this be in" or "from here, what can it become next" without scraping a diagram or driving a deliberately invalid transition to read the refusal text. Previously `lifecycle` named a machine with no catalog to join.

- **A custom agent backend can now declare its per-entry pointer files, and nothing breaks if it doesn't.** `AgentBackend` gains one optional method, `managed_entry_paths(ctx)`, returning the per-entry files this backend expects for the roles and skills that are currently live — one role's pointer, one skill's staging file. Implement it and those files join the new missing-pointer report in `sq check` and `sq sync`; leave it out and the base class answers "none", which is exactly the behaviour a backend written before this release already had. **The seven abstract methods are unchanged**, so a backend written against the documented interface still satisfies it — this addition is opt-in, and the two bundled backends implement it.

### Fixed

- **`sq check` and `sq sync` now tell you when generated agent config is missing or out of step.** A squad whose per-entry backend files were absent — one role's agent pointer, a skill pointer, an `AGENTS.md` staging file — reported nothing at all, so a clone could claim a full roster while the agent host found none of it. `sq check` now flags each missing pointer for a live role or skill at **warn**, so your exit code is unchanged: a missing pointer is recoverable by running `sq sync`, and on its own it does not mean the backend was never set up. A missing *top-level* managed file — `.claude/settings.json`, a compiled `AGENTS.md` — is still an error, as before, so a backend directory that was never scaffolded (or was deleted wholesale) still fails the check. `sq sync` now names each pointer it had to regenerate and which backend owns it, so you know a commit is owed; a healthy sync's output is unchanged. Retiring an entry deliberately removes its pointer and is never reported as a fault.

- **`sq check` now reports a frontmatter/index divergence, and `sq sync` reports it once.** When an interrupted write left an item's markdown and the index disagreeing, `sq sync` would tell you to run `sq repair` while `sq check`, run seconds later, said there were no issues — it compared only two fields, status and parent. It now compares the same fields the write path does, at warn level, so the two commands can no longer disagree about what "diverged" means. `sq sync` no longer prints the same warning twice for one divergence either.

- **A blank or whitespace-only name is refused wherever you can set one.** `sq dev add --name "   "` and `sq role activate <slug> --name "   "` used to succeed, storing a name made of spaces that then rendered as an empty roster line and told the agent it was "the  on this project". The same went for a role override: `full_name = ""`, `title = ""`, `mission = ""` — or any other string field, including `description`, `model`, `color`, or an entry inside `responsibilities`/`agreements` — passed validation silently and reached every generated surface broken, as a blank bold roster line in `CLAUDE.md` and `AGENTS.md`, a blank role-section heading, or an agent pointer file giving the agent no identity at all. All of it is refused now. `sq sync` names the override file and every blank field in it and stops; `sq check` reports the same failure, including for an override file belonging to a role you have not activated, which no sync would have reached yet. An empty `--name` on `sq dev add` no longer falls back to an auto-picked name either, so a mistyped flag cannot quietly do something other than what you asked. Omitting a field is unaffected: it still inherits the base value exactly as before, and only a field set to nothing, or to whitespace alone, is refused.

- **A name made of spaces that an earlier version stored is repaired for you.** Older versions accepted a whitespace-only `--name` and kept it, which left the role showing a blank name in `sq role list`, in your `CLAUDE.md`/`AGENTS.md` roster and in the agent's own pointer file, with `sq check` reporting nothing wrong. The next `sq sync` after upgrading writes a real name back over it — the bundled catalog name for one of the built-in roles, an auto-picked pool name for a developer — in both the role's record and the index. There is nothing to run by hand and nothing to migrate; a second sync changes nothing further. A blank name carries nothing worth keeping, which is why this one can heal itself — a *real* name that an earlier sync replaced with the bundled default is gone and has to be set again. Either way, the way to choose the name yourself is `full_name` in `.overrides/roles/<slug>.toml`.

- **An operator-set agent name now survives `sq sync`.** Naming an agent with `sq init --name <slug>=<name>`, the `[init.names]` table in `.squads.toml`, `sq init`'s interactive prompt, or `sq role activate <slug> --name "…"` used to be reverted by the very next sync — back to the bundled catalog name, in the role's own record, in `sq list`, and in your generated `CLAUDE.md`/`AGENTS.md` roster and `.claude/` pointer — while `sq sync` exited 0 and `sq check` reported nothing wrong. (A developer named with `sq dev add --name` was never affected.) A role's stored name is now authoritative over the bundled default for exactly the field these commands set, while every other catalog field — mission, responsibilities, spawn authority — still refreshes on sync, so an updated bundled role still reaches an item created before the update existed. A `full_name` declared in `.overrides/roles/<slug>.toml` still renames, as before. A rename is visible now too: it moves the role's `updated_at` and is recorded in the reflog. A squad that already lost a name to an earlier sync is not repaired automatically — write the name into `.overrides/roles/<slug>.toml` as `full_name = "…"` and run `sq sync` once.

- **A role override's declared name and mission now reach the role itself, not just its internal record.** Renaming a role (`full_name`) or rewriting its mission (`mission`) in a `.overrides/roles/<slug>.toml` used to update `sq role <slug> show` and your generated `CLAUDE.md`/`AGENTS.md` roster, but leave the role's own record — what `sq list`, `sq show` and `sq search` all read — carrying the old name and mission, so the same squad could show two different names for one role depending which surface you asked. Both now update together, and `sq search` matches the name you declared rather than the one you replaced. A declared one-line `description` now keeps up too: it was applied when the role was first activated and then never again, so editing it in the override left the old text in the generated agent pointer file indefinitely. A squad already carrying a stale name, mission or description heals on its next `sq sync`; nothing to migrate by hand.

- **A partial developer-role override now works everywhere a role override normally does.** A `.overrides/roles/<tech>-dev.toml` that sets only a field or two — a title, a model — used to be refused by `sq sync` and `sq role <slug> show` with a "missing required fields" error, even though `sq dev add` itself already merged that same file field-by-field. Both commands now merge it the same way `sq dev add` does: onto that developer's own current title, model and mission, so an unspecified field simply stays what it already was. `sq check` no longer flags a file shaped this way as broken. Omitting `full_name` always keeps the developer's current name, however they got it (the auto-picked name or one you set with `--name`); setting `full_name` in the override renames them, exactly like overriding any other role's name does. `sq role <slug> show` also stops degrading to a bare three-line card for an activated developer with no override at all — it now renders the same full card (title, model, mission, responsibilities) every other role gets.

- **`sq` messages no longer hard-wrap when you pipe, capture, or read them in a narrow terminal.** A single-line message used to be reflowed to the terminal width, splitting a remedy command or a file path mid-token with an inserted newline — so pasting it pasted two commands, and `grep` over the captured output missed it. Every single-line message `sq` prints now goes out unreflowed: the `error: …` a command prints when it refuses or fails, the refusal a type's command group prints while your workflow override will not load, the schema-mismatch stop that tells you to run `sq migrate up`, the version and sync notices, an invalid `--at` or `sq reflog --since` timestamp, an address given with no verb after it (`sq role manager`), the per-file skip reports from `check`, `repair`, `board list`, `memory list`/`search`, `inbox` and `search`, `sq import`'s per-line diagnostics, and `sq override scaffold`'s guidance. Multi-line panels and tables are unchanged.

- **`sq board list` and `sq memory <role> list`/`search` now report an unreadable file on stderr.** In human-readable mode those per-file skip reports went to stdout, so they polluted piped output and could not be filtered by redirecting stderr — contradicting both the `--json` behaviour and each command's documented contract. **If you parse the human output of these commands, the skip lines are no longer in it**; redirect stderr if you want them back in the same stream.

- **Viewing an item with a lot of sub-entities is dramatically faster.** `sq show <id> --json` on a large review or task used to re-read and re-validate your whole board once per sub-entity it reported — the cost scaled with sub-entity count, and it was the slowest part of opening an item's preview in an editor extension. Every command now reads the board once, whatever it is looking up: `sq show <id> --json`, `sq tree`, `sq list`, `sq check`, `sq sync`, and the addressed form too — `sq review 12 show --json` and its sibling verbs — which used to read it twice regardless of sub-entity count. A review carrying twenty findings now costs what one carrying five does. On a squad with a `.overrides/workflow.toml` the index is additionally checked against the vocabulary your override declares, and that check now runs once per command rather than once per lookup — so such a squad parses the index twice per command, where the heaviest forms previously reached five times. A squad with no override parses it exactly once. Output is unchanged in every case.
