...
- **A `milestone` item type (`MILE`), for the release or cycle work is aimed at.** A records-type item with its own lifecycle (`Draft → InProgress → Done`, plus `Cancelled`) that tracks whether the target is still being pursued, never how much of its work is finished, and a `target_date` set with `sq milestone <n> update --set target_date=…`. Previously a project's releases and cycles had nowhere to live in squads at all.
  ...
  **`sq milestone <n> show` renders the roll-up: members split into delivered and outstanding, each side counted.** That is the first bundled derived view, and the split is read from each member's own status *role* rather than a status name, so a milestone holding items of several types whose lifecycles spell "finished" differently still groups them correctly. It is computed on every request, `--json` included, so there is nothing to refresh. squads has no estimation vocabulary and the roll-up reports item counts accordingly — no burndown, no velocity, no sprint length.
