...
- **`sq repair` removes two stored regions that are now computed on every read.** The sub-entity roll-up table and each block's badge line were snapshots of state the frontmatter already holds; a template-owned skill's stored body is rendered from its template on every `sq skill <slug> show`. All three are removed wherever a corpus still carries them, so repair produces a **content diff** on such a corpus, alongside the index rebuild it is usually run for. It reports the files it rewrote and names them in the reflog; run it on a clean working tree, because it cannot separate its changes from yours. A squad carrying none of them is untouched, no file is written, and a second run is always a no-op.

  **A skill body is emptied, never deleted, and only a template-owned one is touched at all.** The marker pair stays, because an absent region is what `sq skill <slug> show` reads as "no item for this slug". A **custom** skill — one you created yourself — is authored storage and is byte-identical afterwards, including one whose slug starts with `sq-`; so is every sub-entity body and discussion, every item body, and every frontmatter field. The one behaviour that changes with the bytes: text that lived only in a removed region is no longer matched by `sq search`.

  Migrating from 0.11 gets the same removal without running anything extra — it rides the index rebuild at the end of `sq migrate up`, and `sq migrate chlog` describes it. A squad already at schema 0.14 has no migration to run, which is why the cleanup is attached to the ordinary verb instead.

### Migration

**Schema 0.11 → 0.14 — run `sq migrate up`.** No existing item is rewritten and no frontmatter shape changed. The two new item types are declared in the bundled spec already, and the runner supplies the two things a declaration alone never puts on disk: each type's own folder, and its generated agent-facing surface — the managed `sq-contract` and `sq-milestone` skills, their pointers, and the compiled `CLAUDE.md`/`AGENTS.md` regions that reference them. Every write is either a path that did not exist or a region this runner is the first author of, and running it twice changes nothing further.
