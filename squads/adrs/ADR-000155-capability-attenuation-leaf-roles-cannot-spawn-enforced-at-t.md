...
Proposed — parent context EPIC-000121; governs FEAT-000122; resolves BUG-000152.
...
This flat-capability model caused a concrete incident. **BUG-000152** (motivating incident): a
...
2026-06-15 self-review (EPIC-000121) was the same class of failure on a different axis: a session
...
- **Capability** — a leaf specialist *carries* a tool it should not (the Agent/Task spawn tool).
  This is BUG-000152. It needs no verified identity: the constraint binds to the agent **type** at
...
- **Identity / attribution** — a session *claims* a privileged role it was not spawned as (the
  ...
  trustworthy identity and lineage, which **FEAT-000125** owns.
...
1. **Enforcement lives at the Claude Code backend, via per-role tool lists — not an sq-runtime
   ...
   **Boundary caveat.** This binds an agent launched **by type** (`Agent(slug)` / `claude
   ...
   settings. For the BUG-000152 cascade (manager spawns `python-dev` by type) this is exactly the
...
2. **First attenuation cut: "all tools minus Agent/Task."** Spawn authority is reserved to
   ...
   BUG-000152: a leaf that does not hold the tool cannot self-spawn, regardless of how it reasons
   about its task. It is independent of FEAT-000125 and unblocks the bug immediately.

3. **Identity-aware capability profiles are gated on FEAT-000125.** The richer per-role capability
   ...
   trustworthy identity to enforce against, and is explicitly **out of this slice**. FEAT-000125
...
- **Positive.** BUG-000152's recursive-spawn cascade is closed structurally and immediately, with
...
- **Scoping.** FEAT-000122's leaf-no-spawn slice (its US2) ships on type-bound tool lists alone and
  does **not** depend on FEAT-000125; its existing `depends-on FEAT-000125` ref is too strong for
...
- **Negative / limits.** Enforcement does not cover human-driven `--as <role>` main-thread
  ...
  advisory until FEAT-000125 lands a trustworthy identity primitive. The first cut is coarse
