...
- [2026-06-17T20:12:08Z] Pierre Chat:
  - Decision: fix this the structural way — option (2), per-role capability attenuation (withhold the spawn tool from leaf specialists). Tracked as FEAT-122, which we'll pick up very soon. Option (1) instruction-tightening is optional interim mitigation.
- [2026-06-21T22:27:41Z] Mara Tester:
  - QA verification complete. BUG-152 is closed — the recursive self-spawn mechanism is no longer possible for leaf specialist roles.
  ...
  - **Mechanism closed by TASK-156 + ADR-155:** The disallowedTools: Agent entry in the agent frontmatter is bound at the agent TYPE at spawn time — before the child session runs. A leaf specialist cannot invoke the Agent tool regardless of what it reasons or claims; the tool is simply not available in the session. This closes the BUG-152 path.
  - **Scope boundary (not covered by this fix):** A human running a main-thread session --as a leaf role slug in prose is out of scope — that is the identity/Slice B concern (FEAT-125). The boundary is explicit in ADR-155 and TASK-156.
  - Refs: TASK-156 (fix), ADR-155 (decision).
<!-- sq:discussion:end -->
