...
(2) `sq <type> status <X>` validates X against the global `Status` enum, not against that type's workflow. So an out-of-workflow status is ACCEPTED at set-time and only rejected later by `sq check`. Concretely: BUG-000134 was set to `Fixed` (a real enum value), committed, then failed `sq check` (`status 'Fixed' invalid for bug`).
...
**Scope notes:** the bug workflow + status vocabulary are stability-contract surface that freezes at 1.0 (FEAT-000013), so this is pre-1.0 and needs an ADR — including a back-compat/migration plan for existing bugs already in generic-machine statuses (e.g. BUG-134 is now `Done`). Discovered via BUG-000134 during FEAT-000016 QA.
<!-- sq:body:end -->
...
- [2026-06-16T12:00:08Z] Robert Architect:
  - Ruling in ADR-000143 (Accepted). @python-dev implement per its body.
  ...
  - Schema/sequencing: needs a bump. FEAT-000138 owns 0.3→0.4 (already in the working tree, runner _v0_3_to_v0_4 registered). So THIS migration is 0.4→0.5 and MUST land AFTER FEAT-138 merges — bump SCHEMA_VERSION to 0.5, add _v0_4_to_v0_5.py + registry entry, and add a v0_5 corpus fixture wired into test_migration_corpus._CORPUS_CASES (FEAT-17 rule). Do not collide on _schema.py/_registry.py while FEAT-138 is in flight.
  - FEAT-000013 flag: the final bug lifecycle is stability-contract surface freezing at 1.0; it must be recorded in the FEAT-13 doc before freeze. @manager files the deferral comment during the loop.
...
- [2026-06-16T13:49:37Z] Paul Reviewer:
  - APPROVED (REV-000145). Independent review complete; fix conforms to ADR-000143 with the op-pierre override (schema stays 0.3, no migration runner — 9 bugs remapped Done→Verified in place).
