...
<!-- sq:finding:F1:body -->
**File:** `src/squads/_index/_store.py:94-151` (`IndexStore.transaction`) + ADR-000153 Decision 2.
...
- [2026-06-19T12:27:33Z] Robert Architect:
  ...
  - Next: I will re-amend ADR-000153 Decision 2 to specify the per-loop-Lock + threading.Lock + thread_local=False model (and drop the keeper language) once @manager confirms direction. @manager see my report.
...
The whole point of ADR-000153 Decision 2 (option a) is that the lock acquire — which may block up to `lock_timeout` — hops to a worker thread "so the event loop stays responsive." `repair` violates exactly that, and the acceptance criterion is "No blocking ... service call remains anywhere below the CLI entry edge."
...
- [2026-06-18T13:46:00Z] Elias Python:
  - All 6 findings from REV-000154 implemented and marked Fixed. @manager @reviewer
  ...
  - Tooling gates: pyright 0 errors, ruff check clean, ruff format clean. Grep gate clear (no blocking IO under _services/_backends). FEAT-000034 left InProgress.
- [2026-06-19T14:05:55Z] Robert Architect:
  - Decision 2 of ADR-000153 is now DEFINITIVE and CLEAN (re-read it whole — no longer a stack of corrections). It specifies the validated three-layer model (A3) and drops the keeper-thread design entirely. Status stays Accepted; a one-line dated footnote records that it supersedes the 2026-06-18 two-layer amendment and the keeper-thread implementation. @python-dev this is your spec for the F1 fix.
...
- [2026-06-19T14:14:44Z] Elias Python:
  - Replaced keeper-thread design with the three-layer model specified in ADR-000153 Decision 2 (A3). Commit 9fd9a6d on feat/async-core.
...
- [2026-06-19T14:23:04Z] Paul Reviewer:
  ...
  - Spot-checked previously-cleared areas (CLI bridge, await-ordering, atomic write) — unchanged. @manager review Approved, FEAT-000034 unblocked.
<!-- sq:discussion:end -->
