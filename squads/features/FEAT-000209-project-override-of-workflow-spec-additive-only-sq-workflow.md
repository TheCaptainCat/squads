...
Requires F2 (FEAT-000208). The model must accept string-typed vocabulary before 
...
etc.) is delivered by FEAT-000210, not this feature — the CLI item commands are 
...
etc.) are scoped to FEAT-000210, not this feature.
...
- [2026-06-30T07:54:16Z] Olivia Lead:
  - Broke FEAT-000209 into 7 tasks (TASK-000239–245), all parented here, Draft, mapped to US1/US2/US3. Sequenced as a dependency chain: merge loader → service-threading + validate → lint → sq check → override artifact → docs.
    ...
    1. The extra='forbid' prereq from your comment is ALREADY DONE — FEAT-000208/ADR-000232 §5 hardened WorkflowSpec/ItemSpec/StatusSpec/Lifecycle/RefRule with extra='forbid' and routed _loader.py through model_validate. Verified in code. So it is not a standalone task; 239 just re-confirms it. No rework needed.
    
    2. OPEN DESIGN QUESTION (no ADR needed — the destination is already pinned by ADR-000214 §1: "F3+ threads a per-Service instance"): the workflow spec is a module-level singleton consumed at ~16 import sites (CLI, store, all services). Full per-Service threading is a large, risky refactor. For F3's 7 ACs, a narrow squad-dir-aware singleton REBIND at open_service time is sufficient and low-risk. I scoped TASK-000240 around the rebind and told the dev NOT to start the full-threading variant until you/@architect confirm. Decision needed: rebind (recommended) vs. full threading. No new ADR — this is a scope call within the accepted ADR-000214 direction.
...
- [2026-06-30T08:25:59Z] Paul Reviewer:
  - @manager Independent engine-core review of TASK-239/240/241 done → REV-000246, verdict APPROVE (with low/info nits; no blocking changes). I did not write this code.
...
- [2026-06-30T09:13:41Z] Paul Reviewer:
  - @manager Independent review of FEAT-000209 Wave 2-4 done → REV-000248 (new review, linked addresses→FEAT-209 + TASK-242/243/244/245; REV-246 stays as the Wave-1 record). I did not write this code. VERDICT: APPROVE — no blocking defects in the Wave 2-4 deliverables.
...
- [2026-06-30T09:26:25Z] Nina Product:
  ...
  - 1. Override location (AC#1, US1 title, US1 body): corrected from '.squads.toml' to '.overrides/workflow.toml', matching the implemented canonical path. Confirmed by REV-000246 F2, REV-000248 F1, and Mara's final QA notes.
  ...
  - 3. AC#1 / US1 scope boundary (the substantive one): the old wording implied custom types were usable end-to-end. What FEAT-000209 actually delivers: the override file loads and merges additively, the spec is validated and fail-closed, 'sq workflow lint' works, 'sq override scaffold/diff/update workflow' works, and 'sq list -t <customtype>' resolves and filters. Creating/showing/transitioning items of a custom type ('sq create incident', 'sq incident N show', etc.) is delivered by FEAT-000210. Both the feature body AC#1 and the US1 body now say this explicitly with a one-line FEAT-000210 note.
...
- [2026-06-30T09:32:22Z] Catherine Manager:
  - FEAT-000209 complete. Delivered: squad-dir-aware additive-only merge of .overrides/workflow.toml over the bundled default (fail-closed on redefining a built-in); the spec installed via a narrow process-global singleton rebind in open_service; WorkflowSpec.validate() extended with all-cycles parent-cycle detection + live-index cross-check; AC#5 fail-closed wiring (open_service hard-stops on a dropped-but-in-use type/status, listing offenders); sq workflow lint (collect-ALL-errors, incl. every additive conflict in one pass); sq check integration; workflow as the 3rd sq override artifact (scaffold/diff/update); and user docs (sq docs workflow). All 7 tasks Done; REV-000246 (engine core) + REV-000248 (surfaces) Approved; gates clean (pyright 0/0/0, ruff, 169 targeted tests green). Boundary: end-to-end create/use of custom-typed items (sq create incident, etc.) is FEAT-000210 — the static-enum CLI registration is replaced there. Cleanup carried to TASK-000247 (Group B reportPrivateUsage).
<!-- sq:discussion:end -->
