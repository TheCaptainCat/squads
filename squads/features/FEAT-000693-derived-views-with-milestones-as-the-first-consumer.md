...
updated_at: '2026-07-30T07:57:47Z'
---
<!-- sq:body -->
## The problem
...
adopter-declared item types — a custom type with custom edges is only half useful if nothing can
roll them up.
...
- **source** — the relation to project: refs of a given kind pointing at this item, a sub-entity
  collection, a subtree.
...
- **presentation** — how that data renders: a template over the projected records. A table, a
  single badge line, a prose sentence, a bulleted list, a nested outline — all the same mechanism.
  Templating is already how this project renders everything, and the head badge line is already a
  text template over sub-entity state, so no new rendering technology is introduced.
- **sink** — where the rendered result surfaces.

**The sink follows the source; it is not a free choice.** Whether a projection may be written down
is decided by whether the data it projects is local to the item or foreign to it.

- **Local source** — the item's own frontmatter, such as its sub-entity collection — may use the
  `body` sink: materialized into a marker-delimited region, visible to anyone reading the raw
  markdown. The refresh runs inside the same transaction that changed the data, so it cannot
  drift.
- **Foreign source** — refs pointing at this item from other files, or a subtree of other items —
  must use the `display` sink: computed when the item is shown, never written. Materializing it
  would mean every edge change in another file reaching in to rewrite this one: write
  amplification plus a staleness window no lock can close.

The rule describes what the codebase already does rather than adding a constraint. Both
materialized surfaces today — the sub-entity summary and the head badge line — project the
parent's own frontmatter, while `sq tree` and `sq blocked` are foreign-source and have always been
computed on demand. The mechanism should enforce it, so that a foreign-source view cannot declare
the `body` sink at all.

The price, accepted knowingly: a milestone's roll-up is foreign-source, so it cannot be written
into the milestone file and is not visible when reading that markdown on a forge. That is the cost
of the membership edge living on the work item.
...
So `--json` emits the projection and skips presentation entirely. The CLI's table is one
...
membership is recovered by inversion. The milestone's roll-up is a `display`-sink view over that
...
- Showing a milestone renders its members grouped delivered and outstanding with counts.
- `--json` emits that same projection as structured data. The projection is defined once and the
  table is one presentation over it, never the source.
- Presentation covers more than tables: at least one non-tabular presentation ships and is
  exercised — a single-line text rendering is the obvious candidate, since the head badge line
  already needs exactly that.
...
- A foreign-source view cannot declare the `body` sink. The mechanism refuses it rather than
  trusting the author to know better.
- The declaration is expressive enough to describe the existing sub-entity summary as it stands,
  without bending its design to fit.
...
- **Converting the existing sub-entity summary and head rendering** onto the mechanism.
- **Adopter-authored presentation templates.** A built-in presentation set ships here. Letting a
  project supply its own template needs project-level template overrides, which this codebase has
  deliberately deferred; coupling the two would drag an undesigned override surface into this work.
  The presentation layer should be shaped so that path opens later without rework.
<!-- sq:body:end -->
...
<!-- sq:story:US1:body -->
A view declares four things: its source (refs of a given kind pointing at this item, a sub-entity collection, a subtree); its projection (fields, grouping, ordering) which produces structured data and makes no presentation decisions; its presentation, a template over those records; and its sink, constrained by the source rather than freely chosen — local-source views may materialize into the body, foreign-source views are display-only, and the mechanism refuses the other combination. Projected data keeps one uniform shape — records with typed fields, optionally grouped — across every source and presentation. Membership comes from inverting stored forward refs and is never persisted on the projecting item.
...
<!-- sq:story:US2:body -->
One projection, many presentations. `--json` emits the projected records and skips presentation entirely, so the VS Code extension and sq ui lay the data out themselves instead of reimplementing what the CLI prints. The Rich table is one presentation among several — tables, single-line text, lists, nested outlines all render from the same records through a template. Grouping and field metadata travels with the payload so an unfamiliar view can be consumed without special-casing it.
...
<!-- sq:story:US4:body -->
The milestone's members surfaced as a display-sink derived view over the inverted `targets` edges: grouped delivered and outstanding, with counts, because what is left is the question a milestone exists to answer. Nothing is written to the milestone file when membership changes.
