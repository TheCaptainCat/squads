...
second (`sq role show manager`, `sq skill show squads`). FEAT-000019 established uniform addressing
...
Roles/skills/operators are tracked items with sequence IDs (ROLE-000001..., SKIL-000001...,
...
panels. FEAT-000026 introduced styled markdown rendering for all item types (`sq <type> <n> show`
renders headings, bullets, code, panes, `--raw` for opt-out, plain when piped). Role (and skill)
show commands were written before FEAT-000026 landed and were never updated to use the renderer
it built.
...
One grammar, no exceptions. The stability contract (FEAT-000013) documents the CLI grammar as
...
FEAT-000019's spirit. The shared resolver from FEAT-000019 already handles ID and number; the
...
renderer introduced in FEAT-000026 — headed panes, inline markdown (headings, bullets, code),
...
- `--full` / `--comments` flags on role/skill/operator show: bring the body into the FEAT-000026
...
- [2026-06-12T12:07:19Z] Olivia Lead:
  - Carried by TASK-000065 (ST1, ST2, ST5). Exact slug match only — no fuzzy (op-pierre confirmed). Resolution order: full-ID shape, then bare number, then exact slug against extra[X.SLUG] (fallback it.slug). Reuse _role_item/_skill_item/_operator_item in _services/_base.py for the slug lookup.
...
- [2026-06-12T12:07:20Z] Olivia Lead:
  - Carried by TASK-000065 (ST3, ST4). sq role catalog replaces sq role list --available (rows from PREDEFINED). Remove all three list subcommands with no shim. sq dev group is out of scope — leave its list alone, just flag in review if it now looks inconsistent.
- [2026-06-12T12:46:35Z] Paul Reviewer:
...
- [2026-06-12T12:07:21Z] Olivia Lead:
  - Already satisfied by Nina's deferral comment on FEAT-000013 (2026-06-12) — it records all five grammar decisions. Tracked as ST6 on TASK-000065: only top it up if the shipped grammar deviates from that note. No new comment needed if implementation matches.
- [2026-06-12T12:46:36Z] Paul Reviewer:
  - US4 (FEAT-000013 deferral): PASS, no drift. Shipped positive surface (item-first slug|id|n, verb-first creation, sq role catalog, list removal without shim, slug resolution) matches Nina's 2026-06-12 deferral note verbatim, so ST6's top-up correctly did not fire. The AddressDispatchGroup-vs-nested-subgroup choice is internal wiring, not contracted surface — no contract drift on what FEAT-000013 records. Caveat: the negative-path error UX (separate REV) is arguably part of the 'final CLI surface' an operator experiences; if the team wants the migration hint in the contract, note it there — but it does not invalidate US4 as written.
...
- [2026-06-12T11:57:56Z] Pierre Chat:
  ...
  - Second, when I run 'sq role show' the body prints completely plain — raw markdown characters. Feature bodies render nicely now (thanks to the work in FEAT-000026). Role bodies look like they didn't get the update.
...
- [2026-06-12T11:58:11Z] Nina Product:
  - Authoring rationale.
  - Priority set to high (not medium) because this is a grammar question, and FEAT-000013 documents the CLI grammar as SemVer-stable from 1.0. Every surface that isn't deliberately settled before tagging becomes an accidental promise we can never fix cheaply. That's the same reasoning that made FEAT-000019 high — addressing rules are a grammar question, and so is command ordering.
  - Target grammar: 'sq role <slug|id|n> show' — item-first, consistent with every other type. Slug is the natural primary address for roles (analogous to how numbers are for tasks), but full ID and bare number must also work per FEAT-000019's spirit. The shared resolver from FEAT-000019 will need extending or wrapping to cover slug resolution.
...
- [2026-06-12T12:07:09Z] Olivia Lead:
  - Broke this down into two stacked tasks. @python-dev — start with TASK-000065 (grammar + catalog + list removal), then TASK-000066 (styled body rendering); both touch the same three CLI modules (_role.py / _skill.py / _operator.py) plus _common.py, so do them as one PR or a clean stack, 65 first.
  - TASK-000065 covers US1 (item-first slug|id|n addressing, exact match) and US3 (sq role catalog, removal of the three list subcommands + --available, no shim). Includes the resolver extension (exact slug lookup layered on the FEAT-000019 number/id resolver) and the template updates that teach the old grammar (claude_section / greeting / squads skill).
  - TASK-000066 covers US2: route role/skill/operator show bodies through the FEAT-000026 styled path. The seam is print_item in _common.py — it currently guards out ROLE/SKILL bodies (line ~278); relaxing that guard is the preferred fix. Watch for regressions across feature/task/review show.
  ...
  - US4 (FEAT-000013 deferral) is already satisfied — Nina's 2026-06-12 deferral comment on FEAT-000013 records all five decisions verbatim. ST6 on TASK-000065 only fires if the shipped grammar deviates from that note; if it matches, leave it untouched.
- [2026-06-12T12:26:56Z] Elias Python:
  - TASK-000065 delivered. Item-first grammar for sq role/skill/operator is now live with all six subtasks green, full suite (344 tests) passing, pyright strict and ruff clean.
  - Wiring shape: AddressDispatchGroup (TyperGroup subclass, cls= parameter) overrides _click_resolve_command — named subcommands dispatch normally; unknown tokens route to a hidden _addr subgroup that receives the full args list so its callback can consume the address token. The nested address subgroup carries show/regen/rm. This is the probe-confirmed shape; it handles sq role catalog, sq role activate qa, sq role manager show, sq role 1 show, sq role ROLE-000001 show cleanly.
  ...
  - For TASK-000066 dev: the seam for styled body rendering is print_item in _common.py (line ~278 guards out ROLE and SKILL types). The role show command currently calls svc.role_body(slug) and prints plain; TASK-000066 should route through print_item instead. Also note operator show currently has no body rendering (empty no-op as agreed). The _addr subgroup pattern in each CLI module is the hook point for adding --raw/--comments flags to show.
- [2026-06-12T12:38:31Z] Elias Python:
  - TASK-000065 and TASK-000066 are both done. FEAT-000064 is ready for review.
  - TASK-000065 (grammar flip): AddressDispatchGroup dispatches role/skill/operator show/regen/rm through the item-first address subgroup. sq role catalog added; sq role/skill/operator list removed. Templates updated and sq sync run. 344 tests green at handoff.
  - TASK-000066 (styled body rendering): render_body_text() helper added to _common.py. Relaxed print_item guard (ROLE/SKILL/OPERATOR now flow through _print_item_content — sub-entity branches no-op cleanly). show_role, skill_show, operator_show each call render_body_text(body, raw=raw) after their custom panel. --raw flag added to all three show commands. 354 tests green, pyright strict clean, ruff clean.
