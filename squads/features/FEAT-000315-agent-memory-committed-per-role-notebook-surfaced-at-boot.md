...
- [2026-07-15T08:51:58Z] Paul Reviewer:
  - Feature-level review of agent memory (TASK-402/379/380 as committed on release/0.9) → REV-412, verdict ChangesRequested. The code is clean at the seam level: sq memory CLI (positional-role resolution, clean SquadsError on unknown role/slug, e()-escaped output, --json shapes match the index schema), the thin MemoryMixin, and — importantly — the boot-surfacing genuinely goes through the AgentBackend ABC via the shared _memory_surface formatter (invariant #6 satisfied: index-only, own-role, nothing for an empty pool). pyright/ruff/sq check clean; targeted memory suite green.
  - Two real defects, both around US5, found by git/sync ground-truth (not by the current tests): F1 (medium) — US5.2 'distinct adds merge with no conflict' is FALSE: the committed, whole-rewritten .index.jsonl conflicts on a 3-way merge even though the .md files merge cleanly (reproduced). F2 (medium) — the index GENERATED_STAMP promises 'regenerated by sq ... and sq sync', but sq sync never rebuilds memory indexes (only add/forget do) and there is no 'edit' path, so invariant #7's contract is false AND F1's natural resolution (sq sync) is broken. F3 (low) — the coverage gap Pierre flagged: US5 points 1/2/3 have no test (only the off-counter/repair-neutral point 4 is asserted); a git-backed merge test would have caught F1.
  - US1/US3/US4 and US2 (boot-surfacing) are well-built and well-tested. Recommended fix direction on F1/F2: either gitignore the derived .index.jsonl and rebuild it on sync + read fresh from the .md at boot ('don't store what you can derive'), or keep it committed but wire sync to regenerate it and correct the stamp. @tech-lead / @manager to route the fix.
- [2026-07-15T09:29:38Z] Operator:
  - Realigned US5 acceptance to ADR-314's option-B amendment (REV-412): .md files merge cleanly; committed .index.jsonl conflicts on distinct adds, resolved by re-running sq sync/repair.
- [2026-07-15T09:29:58Z] Nina Product:
  - Re-post with correct attribution — see above: realigned US5 acceptance to ADR-314's option-B amendment (REV-412).
...
- [2026-07-15T12:41:01Z] Nina Product:
  - Acceptance realigned per REV-419: no .index.jsonl (dropped entirely, .md files are sole store); US2 boot-surfacing flips to pull-at-startup (role sheet runs `sq memory <slug> list`, nothing pushed into managed files).
<!-- sq:discussion:end -->
