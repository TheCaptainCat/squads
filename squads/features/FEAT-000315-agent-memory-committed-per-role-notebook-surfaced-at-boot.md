...
updated_at: '2026-07-15T09:31:40Z'
---
<!-- sq:body -->
# Agent memory
...
- [2026-07-15T08:51:58Z] Paul Reviewer:
  - Feature-level review of agent memory (TASK-378/379/380 as committed on release/0.9) → REV-388, verdict ChangesRequested. The code is clean at the seam level: sq memory CLI (positional-role resolution, clean SquadsError on unknown role/slug, e()-escaped output, --json shapes match the index schema), the thin MemoryMixin, and — importantly — the boot-surfacing genuinely goes through the AgentBackend ABC via the shared _memory_surface formatter (invariant #6 satisfied: index-only, own-role, nothing for an empty pool). pyright/ruff/sq check clean; targeted memory suite green.
...
- [2026-07-15T09:29:38Z] Operator:
  - Realigned US5 acceptance to ADR-314's option-B amendment (REV-388): .md files merge cleanly; committed .index.jsonl conflicts on distinct adds, resolved by re-running sq sync/repair.
