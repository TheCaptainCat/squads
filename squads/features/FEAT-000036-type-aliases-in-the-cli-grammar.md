...
- **Contract**: the alias table joins the stability doc (FEAT-13) as frozen grammar; adding
...
- [2026-06-15T07:43:38Z] Olivia Lead:
  - Broke FEAT-36 into two tasks. TASK-107 (US1) — alias routing/equivalence: a single canonical ALIASES map next to WORK_TYPES, each type sub-app re-registered under its hidden alias names in _cli/__init__.py (the proven hidden=True _addr pattern), plus a CLI test matrix proving deep-chain equivalence and canonical output/errors.
  - TASK-108 (US2) — help cleanliness + docs + contract: hide aliases from root --help, mention the table once (epilog or type help), render it from the same ALIASES map into the workflow cheatsheet with the add-only evolution rule, and tie into FEAT-13 for docs/stability.md (refd as related; FEAT-13 still owns creating that file).
  - Single source of truth for the table = the ALIASES map TASK-107 introduces; TASK-108 consumes it so routing and docs can't drift. @python-dev TASK-107 is ready to start (TASK-108 builds on its ALIASES map, so take 107 first).
- [2026-06-15T08:07:51Z] Mara Tester:
  - QA verification of FEAT-36 — overall PASS with one minor clarification needed.
...
- [2026-06-15T08:10:19Z] Theo Writer:
  ...
  - The section includes the full alias table, equivalence note (full verb-chain and sub-entity support), output canonicalization rule, and the add-only evolution rule with FEAT-13 tie-in.
