...
- [2026-07-15T14:43:32Z] Paul Reviewer:
  - Reviewed TASK-400 (fenced sq graph) + TASK-401 (spec-derived diagrams). APPROVED — no findings. US2 correctly out of scope (Cancelled per ADR-398).
  - TASK-400: 'mermaid-md' added as a --format value (single-option surface, per task guidance), wraps graph_to_mermaid in a ```mermaid fence, printed markup=False; serializer untouched; invalid --format rejected. CLI smoke test asserts the fence lines.
  - TASK-401: genuinely spec-driven — hierarchy flowchart from parent_chain/spec.items, per-type stateDiagram-v2 from lifecycle_edges/lifecycle_states_in_order. Verified on a renamed vocab (task->ticket) and a custom lifecycle: real names show, no bundled literal leaks. Determinism handled correctly — helpers BFS from initial over the ordered transition lists and never iterate the Lifecycle.states frozenset, so render is byte-stable (test asserts repeated renders equal; golden stable). mermaid_id sanitizes to [A-Za-z0-9_] with the real name kept as the node label. Degrades to a plain fenced block in the Rich terminal (sq workflow renders, exit 0). ADR-398 honored: workflow.md.j2 only, not docs/; regenerated by sq sync; templates_manifest regenerated.
  - Gates: ruff/pyright/format clean, sq check clean, workflow + golden + agents_md tests green (agents_md/search goldens additive/documented-shape). @tech-lead TASK-400 + TASK-401 verified — clear to close.
<!-- sq:discussion:end -->
