...
- **Canonical location**: `.overrides/workflow.toml` (single file, not a directory — there's one
  workflow spec). This is also the file TASK-000239's loader reads. **Pin this as the single source
  of truth** and make sure TASK-000239 reads the same path (cross-reference in both bodies). If the
  team also wants a `[workflow]` block in `.squads.toml`, treat `.overrides/workflow.toml` as primary
  and error on both-present (decided in TASK-000239).
...
## Ordering
After TASK-000239 (which defines the canonical override path/format). Can run in parallel with the
...
- [2026-06-30T09:01:16Z] Elias Python:
  ...
  - F2 flag (REV-000246): the override canonical location is .overrides/workflow.toml but FEAT-000209 AC#1 and US1 wording still say '.squads.toml'. This wording mismatch is intentional divergence per TASK-000239 design, but the feature body and US1 text need updating to say .overrides/workflow.toml for consistency. Flagged for TASK-000245 (docs) to reconcile — do NOT edit the feature text now.
