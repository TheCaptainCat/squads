...
Build the `WorkflowSpec` tree of pyright-strict pydantic v2 value objects per ADR-000214 §1, and
author the bundled-default TOML that encodes today's exact workflow vocabulary. This is the data +
shape foundation for FEAT-000207 (F1) — **enums intact**: enum-typed fields stay enum-typed; the
spec is a reorganization of today's literals into one object, not a de-typing.

Sequence: **first** task. The loader/rewire (TASK-000216) and golden-lock/packaging (TASK-000217)
...
- **Bundled TOML** at `src/squads/_workflow/default_workflow.toml` (promote `_workflow.py` into a
  ...
  (packaging *verification* is TASK-000217). Encode, byte-for-byte vs today:
...
4. pyright/ruff clean. (Equality-with-today is asserted by TASK-000217's golden test.)
<!-- sq:body:end -->
