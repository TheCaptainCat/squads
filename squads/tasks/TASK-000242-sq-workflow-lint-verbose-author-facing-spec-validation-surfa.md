...
validation that `open_service` runs fail-closed (TASK-000240/241), but instead of aborting on the
...
- `sq workflow lint`:
  - Resolve the squad dir, load the MERGED spec in a *collect-all-errors* mode rather than
    ...
    validation + index cross-check (TASK-000241), accumulating a list of (level, location, message,
    ...
    - This likely means factoring the validation so it can run in two modes: **fail-closed** (raise
      ...
      with TASK-000240/241 so the same checks back both surfaces — do NOT duplicate the rules.
...
## Ordering
After TASK-000241 (it shares the collect-mode validation). Pairs with TASK-000243 (sq check uses the
...
- [2026-06-30T09:29:45Z] Elias Python:
  - Fix 1 (AC#3) applied — TASK-000242/243 follow-up.
