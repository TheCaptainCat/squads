...
updated_at: '2026-07-22T18:42:50Z'
---
<!-- sq:body -->
Implements FEAT-605 US1. Drop the explicit `terminal` field and re-express terminal / open / default-visibility as reads of the referenced role object. No expressiveness loss — `settled` + `hidden` on the role carry what a single `terminal` flag plus a category branch would.
...
- `WorkflowSpec.hidden_by_default(item_type, s)` -> `roles[statuses[s].role].hidden` (no category branch).
...
- `_check_reachable_terminal` lint -> "every lifecycle must reach a status whose role is settled"; drop the "terminal status not in status set" belt-check.
- `hidden_by_default` is role-derived for every category; retired-ness is `role.settled`/`role.hidden` off the catalog, with no hardcoded status-role set. `CATEGORIES` (the `get_args` tuple) and the category axis are orthogonal and stay.

## Acceptance
- No `terminal` field on `StatusSpec`; `is_open`/`terminal_set()`/`TERMINAL` all derive from role.
- Bundled default visibility preserved except the one deliberate change: `Accepted`/`Published` shown, `Done`/`Verified` hidden, `Superseded`/`Deprecated` hidden, and `Rejected` now hidden (was shown).
