...
FEAT-138; the runtime/config-consumer half is TASK-140.
...
- **Corpus fixture** (FEAT-17 standing rule, `tests/fixtures/corpus/README.md`):
  ...
  new runner is actually exercised (not vacuous — REV-130 lesson).

## Coordination

The config-model change (`SquadsConfig.default_backend` → `active_backends`)
lives in TASK-140 and is the shared seam: this task's migration writes the new
TOML key, TASK-140's model reads it. Land 139's schema bump + runner first (or
...
- [2026-06-16T12:27:21Z] Elias Python:
  - Implemented TASK-139 (schema 0.3→0.4 migration) per ADR-141.
...
- [2026-06-16T12:43:45Z] Catherine Manager:
  - Cancelled per op-pierre (2026-06-16): we stay on schema 0.3 — no 0.4 bump, no migration. The active_backends change folds into the in-development 0.3 schema via the config's transparent legacy default_backend read (delivered in TASK-140), so this migration task is void.
<!-- sq:discussion:end -->
