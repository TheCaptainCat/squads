...
### The startup-ordering problem (decided in ADR-000263)
...
**Approach: implement Option 3 from ADR-000263** — a lazy-dispatch `TyperGroup`
...
resolves are settled in ADR-000263 — follow it; do not re-litigate the choice.
...
- src/squads/_cli/__init__.py (app-build loop + alias registration; the
  lazy-dispatch `TyperGroup` per ADR-000263), src/squads/_models/_enums.py
...
- Implements ADR-000263 (the startup-ordering decision).
<!-- sq:body:end -->
...
- [2026-06-30T13:20:48Z] Catherine Manager:
  - Cleared the stale 'THIS TASK IS BLOCKED ON THE ADR' body banner — ADR-000263 was accepted (Option 3, lazy-dispatch TyperGroup) after that prose was written, so the banner was contradicting reality and read as a live blocker. Body now states the decided approach; linked ADR-000263 (implements). TASK-257 is unblocked and ready to build behind TASK-256/258 (both Done).
- [2026-06-30T14:26:49Z] Elias Python:
  - @manager TASK-000257 is Done. Here is a summary of every change landed.
