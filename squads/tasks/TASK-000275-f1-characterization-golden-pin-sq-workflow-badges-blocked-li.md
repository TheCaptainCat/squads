...
FEAT-220/REV-000230 process rule: the proof is the first task, not a trailing one an agent can
...
1. **`sq workflow` cheatsheet** — full rendered `workflow.md.j2` output for the **bundled**
   ...
   hardcoded `workflow_static.md.j2` retype-target list, so TASK-000279's spec-derivation of
...
2. **Status badges** — for every built-in item status AND every sub-entity status, pin the
   ...
   column for a built-in item, plus a sub-entity head badge, so TASK-000276 cannot silently
...
- This task's tests are the guard TASK-000276/277/278/279 run under; they must stay green
  through the whole feature.

## Notes for implementer
- Existing golden scaffolding: `tests/test_workflow_renderer_261.py`, `tests/test_golden_json.py`.
- Do NOT pin the FEAT-013 static prose as "spec-derived" — it stays literal (see TASK-000279).
<!-- sq:body:end -->
