...
extra:
  slug: sq-decision
path: agents/skills/SKILL-000194-sq-decision.md
description: 'Working with decision items in this squad: lifecycle, commands, and
...
`sq decision <n> show`, never by opening the file. Items are addressed as
`sq decision <n> <verb>`. Set this item's body with `sq decision <n> body
-m "…"` (or `--file`); `--desc` sets only the short summary. Read anything back with `sq decision <n> show --full --comments` (full dossier, including discussion).
