...
Before you start, run `sq memory devops list`, then `sq memory
devops show <slug>` for anything relevant, and check `sq board list`
...
other: `sq mine devops` for the items assigned to you, and
`sq inbox devops` for the comment lines that `@mention` you.
