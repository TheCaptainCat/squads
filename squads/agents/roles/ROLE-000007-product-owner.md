...
Before you start, run `sq memory product-owner list`, then `sq memory
product-owner show <slug>` for anything relevant, and check `sq board list`
...
other: `sq mine product-owner` for the items assigned to you, and
`sq inbox product-owner` for the comment lines that `@mention` you.
